...
int main( int argc, char** argv )
{
	...
	/* media input */
  VideoCapture cap;
  string path = "/ibox/eboxcopy/recordings/datasets/rgbd_dataset_freiburg1_xyz-rgb.avi";
  //arguments
  if ( argc > 1 )
  {
    cap.open(argv[1]); //open video file in argument
  }
  else
  {
    cap.open(path);
  ...
  cout << "Number of frames in the video = " << frnb << endl;
	// /* TCP Server Setup */
